...
/* ---------------- Nav / role config ---------------- */
const ROLE_NAV = {
  admin:   [['dashboard','📊','Dashboard'],['students','🎓','Students'],['fees','💰','Fees & Finance'],['attendance','📋','Attendance'],['notifications','✉️','Notifications'],['audit','🛡️','Audit Log'],['settings','⚙️','Settings']],
  bursar:  [['dashboard','📊','Dashboard'],['fees','💰','Fees & Finance'],['students','🎓','Students']],
...
async function renderDashboard(){
  try{
    ...
    if(d.role==='parent'){
      cards.push(card('Fee Balance', money(d.balance), d.student? d.student.name : '', d.balance>0?'brick':''));
      cards.push(card('Attendance Record', d.attendance.total? `${d.attendance.present}/${d.attendance.total}` : 'No data yet', 'Days present / recorded', 'gold'));
    }
...
function openAdmitModal(){
  ['admName','admGuardianName','admGuardianPhone','admGuardianEmail'].forEach(id=> document.getElementById(id).value='');
...
async function renderParent(){
  ...
  try{
    const data = await api('/parent/me');
...
async function createUser(){
  ...
  try{
    ...
    ['newUserUsername','newUserPassword','newUserName'].forEach(id=> document.getElementById(id).value='');
  }catch(e){ toast(e.message, true); }
}
